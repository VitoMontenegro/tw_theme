...
<section class="bg-[#DED7FC]">
	<div class="container mx-auto">
		<div class="flex justify-between items-center">
						...
						<li class="group relative py-[14px] lg:py-[20px] md:items-start lg:items-center flex items-center lg:gap-2<?php echo is_active_category($category) ? ' active' : ''; ?>">
							...
							<ul class="submenu absolute top-[60px] bg-[#E9E4FF] w-full px-4 py-4 z-10  flex-col gap-3 border hidden group-hover:flex rounded-md">
								<?php foreach ($category["children"] as $child) : ?>
									<li>
										<a href="<?php echo esc_url($child['link']) ?>" class="font-semibold p-1.5 rounded-md<?php echo is_current_category($child["id"]) ? ' bg-[#C2B6FF] text-[#393488]' : ''; ?> hover:bg-[#C2B6FF] text-[#393488]">
			...
			<?php endif; ?>
			<form class="search-form flex px-4 py-2 bg-white items-center rounded-xl border border-[#B8B9B9] w-full md:w-[180px] lg:w-[240px] my-2 md:my-auto">
				...
				<input type="text" placeholder="Что вы ищете?" class="md:max-w-[113px] lg:max-w-[178px]">
			</form>
		</div>
	</div>
